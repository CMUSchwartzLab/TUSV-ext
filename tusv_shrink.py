...
def check_valid_input(Q, G, A, H, F_full):
	...
	#print(np.shape(Q))
	#print(np.shape(G))
	#print(np.shape(F_full))

	to_be_del = np.where(np.sum(Q,1)==0)[0]
	Q = np.delete(Q, to_be_del, axis=0)
	#print(np.shape(Q))
	#print(np.sum(Q,1))
	#print(len(np.sum(Q,1)))
	#print(np.shape(G))
	G = np.delete(G, to_be_del, axis=0)
	G = np.delete(G, to_be_del, axis=1)

	F_full = np.delete(F_full, to_be_del, axis=1)
	#print(np.shape(F_full))
	#print(np.shape(G))
	#print(np.sum(G, 0), np.sum(G, 1))

	to_be_del = np.where(np.sum(G,0)==1)[0]
	#print(to_be_del)
	...
	while np.shape(Q)[0] >= 200:#180#90
		#Q: (n_sv, n_cnv); G: (sv, sv); F_full: (1, sv+cnv)
		#print(np.shape(Q),np.shape(G),np.shape(F_full))
		to_be_del = np.random.randint(np.shape(Q)[0])
		Q = np.delete(Q, to_be_del, axis=0)
		G = np.delete(G, to_be_del, axis=0)
		G = np.delete(G, to_be_del, axis=1)
		F_full = np.delete(F_full, to_be_del, axis=1)
