...
def main2(argv):
	...
	write_readme(args['output_directory'], args)
	unmix(args['input_directory'], args['output_directory'], args['num_leaves'], args['c_max'], args['lambda1'], args['lambda2'], args['restart_iters'], args['cord_desc_iters'], args['processors'], args['time_limit'], args['metadata_file'], args['num_subsamples'], args['overide_lambdas'])
	upper_directory = args['input_directory'].rsplit('/', 1)[0]
	U = np.loadtxt(upper_directory + '/U.tsv', delimiter='\t')
	C = np.loadtxt(upper_directory + '/C.tsv', delimiter='\t')
	F = np.loadtxt(upper_directory + '/F.tsv', delimiter='\t')
	record_true_obj(args['input_directory'], args['num_leaves'], args['lambda1'], args['lambda2'],  args['num_subsamples'], args['overide_lambdas'], U, C, F)
...
def record_true_obj(in_dir, n, lamb1, lamb2, num_seg_subsamples, should_overide_lambdas, U_true, C_true, F_true):
    F_full, F_phasing_full, Q, G, A, H, bp_attr, cv_attr = gm.get_mats(in_dir)
    Q, G, A, H, F_full, F_phasing_full = check_valid_input(Q, G, A, H, F_full, F_phasing_full)
    ...
    with open("true_objective", 'w') as g:
        g.write(str(obj_val))
